...
public class SystemVerilogMainProcessor {
    ...
    public int processFile(String fileName, ANTLRInputStream ais) {
        ...
        SystemVerilogLexer lexer = new SystemVerilogLexer(ais);
	CommonTokenStream tokenStream = new CommonTokenStream(lexer);
	SystemVerilogParserParser parser = new SystemVerilogParserParser(tokenStream);

	// Setup error reporting
	lexer.removeErrorListeners();
	UnderlineErrorListener lexerUnderlineErrorListener = new UnderlineErrorListener(lexer);
	lexer.addErrorListener(lexerUnderlineErrorListener);

	parser.removeErrorListeners();
	UnderlineErrorListener parserUnderlineErrorListener = new UnderlineErrorListener(lexer);
	parser.addErrorListener(parserUnderlineErrorListener);
